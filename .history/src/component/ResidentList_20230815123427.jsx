...
export const ResidentList = ({ residents }) => {
  const RESIDENTS_PER_PAGE = 9; // Define la cantidad de residentes por página
  const [currentPage, setCurrentPage] = useState(1); // Estado para almacenar la página actual

  const totalPages = Math.ceil(residents.length / RESIDENTS_PER_PAGE); // Calcula el número total de páginas
  ...
  const showIndexPagination = totalPages > 3; // Determina si se mostrará la paginación de índice
