...
export const ResidentList = ({ residents }) => {
  ...
  return (
    <section className="absolute justify-center">
            ...
            {Array.from({ length: 3 }, (_, index) => index + 1).map((index) => (
              <div
                ...
                className={classnames(
                  "rounded-full w-24 h-24 border-8 text-center font-nunito font-semibold text-2xl leading-11",
                  {
                    "bg-opacity-50": currentPage === index,
                    "bg-opacity-25 hover:bg-opacity-50 cursor-pointer transition-opacity":
                      currentPage !== index,
                  }
                )}
